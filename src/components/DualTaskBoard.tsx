...
export const DualTaskBoard: React.FC<Props> = ({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 flex-1 items-start">
      ...
      <div className="p-5 rounded-2xl bg-[#11131a] border border-white/[0.08] flex flex-col gap-4 min-h-[420px]">
        {/* Column Header */}
        ...
        <div className="flex flex-col gap-2.5 flex-1">
          <AnimatePresence mode="popLayout" initial={false}>
            {pendingTasks.length === 0 ? (
              <div className="flex flex-col items-center justify-center p-8 text-center border border-dashed border-zinc-800 rounded-xl my-auto text-zinc-500">
          ...
          <AnimatePresence mode="popLayout" initial={false}>
            {doneTasks.length === 0 ? (
              <div className="flex flex-col items-center justify-center p-8 text-center border border-dashed border-zinc-800 rounded-xl my-auto text-zinc-500">
                ...
                <span className="text-[11px] text-zinc-600 mt-1">Haz clic en una tarea para completarla</span>
              </div>
            ) : (
              doneTasks.map((task) => (
                <TaskCard
