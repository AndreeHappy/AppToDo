...
export const LiquidNavigator: React.FC<Props> = ({
  ...
  // Determine active tab identifier
  const activeKey = activeModule; // 'hub' | 'todo' | 'finance'

  return (
    <div className="fixed bottom-3 inset-x-0 z-40 flex justify-center px-4 pointer-events-none select-none">
      <div className="pointer-events-auto relative w-full max-w-md bg-[#11131a] border border-white/[0.12] rounded-3xl shadow-[0_12px_36px_rgba(0,0,0,0.45)] px-2 py-2 flex items-center justify-around transition-colors">
        {/* Item 1: Hub / Inicio */}
        <button
        ...
        >
          ...
          <span className={`text-[10px] font-bold tracking-tight mt-auto ${activeKey === 'hub' ? 'text-indigo-400' : 'text-zinc-400'}`}>
            Hub
          </span>
        </button>

        {/* Item 2: Alternar Módulo (To-Do / Finanzas) */}
        <button
        ...
        >
          ...
          <span className={`text-[10px] font-bold tracking-tight mt-auto ${
            ...
            {activeKey === 'todo' ? 'To-Do' : activeKey === 'finance' ? 'Finanzas' : 'Módulos'}
          </span>
        </button>

        {/* Item 3: Acción Central Prominente (+) */}
        <button
          onClick={onQuickAction}
          title={activeModule === 'finance' ? 'Registrar Movimiento' : activeModule === 'todo' ? 'Agregar Tarea' : 'Acción Rápida'}
        ...
        >
          ...
          <span className="text-[10px] font-bold tracking-tight mt-auto text-zinc-400">
            Perfil
          </span>
        </button>
